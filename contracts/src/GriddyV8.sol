...
/// @title GriddyV8 — variable-stake native-token pari-mutuel 5x5 grid, full-length rounds
...
///         lobby, they just report the next tick rather than a deadline. Fee/
///         tip/payout math, continuous resolution and every accounting
///         invariant are unchanged from V6. UUPS-upgradeable (storage layout
///         frozen from V2: retired vars retained, unused; V7 appends
///         roundEpoch at the end; V8 appends nothing — pure logic change).
contract GriddyV8 is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable {
    ...
    ///      can render an honest countdown for free. Read the idle branch as a
    ///      "next tick" indicator ONLY, not as a promise: under V8 a stake
    ///      opens a FULL-LENGTH round ending roundDuration seconds after it
    ///      lands, which is normally later than the tick reported here. The
    ///      live branch is the exact deadline; the idle branch is the beat of
    ///      the lobby clock. Either way, a stake is still the first thing that
    ///      writes any storage.
    ...
    ///         in it.
    /// @dev Used by currentWindow only — _startNewRound stopped consulting the
    ///      grid in V8, where every round is a full roundDuration long. This
    ///      keeps the idle lobby's countdown off a sliver of a window.
    ...
    function _startNewRound() internal {
        ...
        // V8: FULL length, always. A round opened one second before a grid
        // boundary is worth exactly as much betting time as one opened right
        // after it — no round ever inherits a window's leftover seconds.
