...
/// @title GriddyV9 — variable-stake native-token pari-mutuel 5x5 grid, grid-aligned rounds
...
///         nothing until the first stake writes one. V8 restores FULL-LENGTH
///         rounds: a round opened mid-window no longer inherits only that
///         window's remainder, it always runs exactly roundDuration seconds
///         from the moment it opens, so a player staking with 20 seconds left
///         on the grid clock still gets a whole round. The grid itself is
///         untouched — roundEpoch and {currentWindow} still tick for an empty
///         lobby, they just report the next tick rather than a deadline. V9
///         puts the clock back on the grid and keeps V8's anti-sliver
///         guarantee: _startNewRound snaps to roundEpoch + k * roundDuration
///         (rolling to the next slot when under MIN_BET_WINDOW seconds remain),
///         so a round's deadline no longer depends on the moment somebody
///         staked. The countdown is a pure function of the clock — identical
...
///         retained, unused; V7 appends roundEpoch at the end; V8 and V9
///         append nothing — pure logic changes).
contract GriddyV9 is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable {
    ...
    /// @notice Shortest grid tick {currentWindow} will advertise to an idle
    ///         lobby: with fewer than this many seconds left it reports the
    ///         NEXT tick instead of a sliver. It no longer bounds how a round
    ...
    ///      but on a fresh deployment initialize() already anchored the grid
    ...
    /// @notice While a round is live: that round's own window and pin — the
    ///         exact deadline a stake sent right now would buy into. While the
    ///         lobby is idle: the next tick of the round grid, derived purely
    ///         from block.timestamp, roundEpoch, roundDuration and beaconGap,
    ///         with no round materialised on-chain.
    ...
    ///      can render an honest countdown for free. Under V9 both branches
    ///      report the SAME thing — a grid slot — because _startNewRound now
    ///      snaps to the grid too. So the idle branch is a promise, not just a
    ///      hint: a stake landing now opens a round ending exactly at the
    ///      windowEnd reported here. (V8's caveat, that a stake opened a
    ///      full-length round ending later than the advertised tick, is gone.)
    ///      A stake is still the first thing that writes any storage.
    function currentWindow() external view returns (
        ...
        // A stake only opens a NEW round when the current one has closed.
        ...
        // does not actually get (under V8 a live round almost never ends on a
        // grid boundary, since it runs a full roundDuration from whenever it
        // opened).
    ...
    ///         or the next one when fewer than MIN_BET_WINDOW seconds are left
    ///         in it.
    /// @dev Used by BOTH currentWindow and _startNewRound in V9, which is what
    ///      makes the advertised countdown and the round actually opened agree
    ...
    function _startNewRound() internal {
        currentRoundId++;
        // V9: boundaries come from the grid, never from the clock reading of
        // whoever staked first. The lobby's countdown is therefore a pure
        // function of (roundEpoch, roundDuration, block.timestamp) — a client
        // can render it with zero chain reads and it keeps ticking through
        // rounds nobody plays. _bettableWindow rolls to the next slot when
        // fewer than MIN_BET_WINDOW seconds remain, so the sliver-round that
        // V8 set out to kill still cannot happen: a stake either buys a
        // meaningful piece of this slot or opens the next one.
        (uint64 start, uint64 end) = _bettableWindow(uint64(block.timestamp));
        uint64 drandRound = beacon.roundAt(uint256(end) + beaconGap);
        // Fairness invariant carried over from V7: the pinned beacon must not
