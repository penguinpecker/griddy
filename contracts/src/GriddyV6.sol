...
/// @title GriddyV6 — variable-stake native-token pari-mutuel 5x5 grid, continuous rounds
...
///         at up to $1 on a USDC-gas chain; the active minimum is still
///         whatever setMinStake last set, and nothing else changes. Pure logic
///         upgrade: no new storage, no reinitializer. UUPS-upgradeable (storage
///         layout frozen from V2: retired vars retained, unused).
contract GriddyV6 is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable {
    ...
    ///         now < endTime ? currentRoundId : currentRoundId + 1).
